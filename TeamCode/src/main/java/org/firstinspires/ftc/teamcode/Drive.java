    ...
    public void runOpMode() {
        ...
        if (opModeIsActive())
        {
            while (opModeIsActive())
            {
                ...
                //Override regulation
                if(gamepad2.dpad_down || gamepad1.dpad_left) // go down(arm)
                {
                ...
                if(gamepad2.dpad_up || gamepad1.dpad_right) // go up(arm)
                {
                    activate = true;//reset lift
                ...
                if(override>0) // if we gave auto control running (override == 1 or 2), and it has not been interrupted by manually switching off the override, we run it.
                {
                    ...
                    if(activate)
                    {
                        ...
                        //stop();
                        if(direction==false)lift_power=-1;
                        if(direction==true) lift_power=1;
                    }

                    if(time.time()-currentTime > delay && activate)
                    {
                        lift_power=0;
                        dump_position=1;
                        activate=false;
